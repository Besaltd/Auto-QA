...
from authenticator import AuthApi
...
def create_test_employee_data():
    ...
    return {"first_name": "John", "last_name": "Lennon", "middle_name": "test", "company_id": 1,
        "email": f"john_{unique_part}@example.com", "phone": f"12345{unique_part[:5]}", "birthdate": str(date.today()),
        "is_active": True}
...
def create_employee_and_get_id(api, employee_data):
    ...
    assert create_response.status_code == 200, (f"Помилка при створенні працівника. "
                                                f"Статус: {create_response.status_code}, тіло: {create_response.text}")

    employee_in_list = api.find_employee_in_company_list_by_email(employee_data["company_id"], employee_data["email"])
    assert employee_in_list is not None, "Працівник не найден"

    employee_id = api.find_employee_id_by_email(employee_data["email"])
    assert employee_id is not None, "Не вдалось знайти ID працівника"
...
def get_user_token():
    ...
    assert response.status_code == 200, (f"Помилка авторізації. Статус: {response.status_code}, тіло: {response.text}")
...
def test_create_employee():
    ...
    assert response.status_code == 200, (f"Помилка при створенні працівника. "
...
def test_get_employee_by_id():
    ...
    assert response.status_code == 200, ("error response by id"
...
def test_update_employee():
    ...
    updated_data = {"last_name": "updated", "email": employee_data["email"], "phone": employee_data["phone"],
        "is_active": False}

    updated_response = api.update_employee(employee_id, user_token, updated_data)

    assert updated_response.status_code == 200, ("error by employee updating"
