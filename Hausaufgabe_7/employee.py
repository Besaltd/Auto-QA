...
class EmployeeApi:
    ...
    def get_employee_by_id(self, employee_id):
        response = requests.get(self.url + f"/employee/info/{employee_id}")
        return response

    def update_employee(self, employee_id, user_token, updated_data):
        response = requests.patch(self.url + f"/employee/change/{employee_id}", params={"client_token": user_token},
            json=updated_data)
        return response

    def get_company_employee_list(self, company_id):
        response = requests.get(self.url + f"/employee/list/{company_id}")
        return response

    def find_employee_in_company_list_by_email(self, company_id, email):
        response = self.get_company_employee_list(company_id)
        assert response.status_code == 200, (f"Не вдалось отримати список працівників. "
